...
class Tournament(commands.Cog):
    ...
    async def faq(self, ctx):
        data = [["What formats are supported?",
                ...
                ["How to setup a tournament?",
                 f"To setup a tournament type `{PREFIX}tournament setup x y` where x is an integer in the range 0 to 2 (Single Elim, Double Elim, Swiss) denoting tournament type and y is the name of the tournament."],
                ["How to register/unregister for the tournament?",
                 f"To register type `{PREFIX}tournament register` and to unregister type `{PREFIX}tournament unregister`. Admins can forcefully unregister a user by typing `{PREFIX}tournament _unregister <handle>`."],
                ["How to start the tournament?",
                 f"To start the tournament, type `{PREFIX}tournament begin`"],
                ["How to compete once the tournament has started?",
                 f"To compete, use the `{PREFIX}round` command and challenge your opponent. The bot will automatically ask you if you want the result of the round to be counted in the tournament and update the tournament bracket once the round is complete."],
                ["What if my opponent doesn't show up or leaves the server without completing the matches?",
                 f"You can ask an admin to use the command `{PREFIX}tournament forcewin <handle>` where handle is the winners codeforces handle."],
    ...
    async def begin(self, ctx):
        ...
        if tournament_info.status == 2:
            await discord_.send_message(ctx, f"The tournament has already begun! Type `{PREFIX}tournament matches` or `{PREFIX}tournament info` to view details about the tournament")
            return
        ...
        if tournament_info.status == 0:
            ...
            desc += f"Note that the tournament has **not** started yet. Once everyone has made their predictions, type `{PREFIX}tournament begin` for the tournament to officially begin\n\n"
    ...
    async def matches(self, ctx):
        ...
        if not tournament_info:
            ...
            return
        if tournament_info.status != 2:
            await discord_.send_message(ctx, f"The tournament has not begun yet, type `{PREFIX}tournament begin` to start the tournament")
            return
    ...
    async def match_invalidate(self, ctx, idx: int):
        ...
        if tournament_info.status != 2:
            await discord_.send_message(ctx,
                                        f"The tournament has not begun yet, type `{PREFIX}tournament begin` to start the tournament")
            return
